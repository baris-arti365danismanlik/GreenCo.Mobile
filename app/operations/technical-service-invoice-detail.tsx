...
export default function OperationsTechnicalServiceInvoiceDetailScreen() {
  ...
  const approveInvoice = async () => {
    if (Platform.OS === 'web') {
      if (!window.confirm('Bu hakedişi onaylamak istediğinizden emin misiniz?')) {
        return;
      }
    } else {
      Alert.alert(
        ...
        [
          ...
          { text: 'Onayla', onPress: () => performApproval() },
        ]
      );
      return;
    }
  ...
  const performApproval = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        window.alert('Hakediş onaylandı');
        router.back();
      } else {
        Alert.alert('Başarılı', 'Hakediş onaylandı', [
          {
            text: 'Tamam',
            onPress: () => router.back(),
          },
      ...
      if (Platform.OS === 'web') {
        window.alert('Hakediş onaylanırken hata oluştu');
      } else {
        Alert.alert('Hata', 'Hakediş onaylanırken hata oluştu');
      }
    } finally {
      setProcessing(false);
    }
  ...
  const performRejection = async () => {
    if (!rejectionReason.trim()) {
      if (Platform.OS === 'web') {
        window.alert('Lütfen red nedeni girin');
      } else {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        window.alert('Hakediş reddedilirken hata oluştu');
      } else {
        Alert.alert('Hata', 'Hakediş reddedilirken hata oluştu');
      }
    } finally {
      setProcessing(false);
    }
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Hakediş Reddi</Text>
